...
int	ft_put_hexadecimal(double nbr, char *base, double size);
int	ft_put_pointer_hex(unsigned long long nbr, char *base,
...
int	ft_printaux(const char format, va_list args)
{
	...
	if (format == 'x')
		return (ft_putnbr(va_arg(args, unsigned int), "0123456789abcdef", 16));
	if (format == 'p')
		return (ft_put_pointer_hex(va_arg(args, unsigned long long),
				"0123456789abcdef", 1));
	return (0);
}
